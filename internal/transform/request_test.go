...
func TestAnthropicToOpenAI_ToolResultTextOnly(t *testing.T) {
	...
	out, err := AnthropicToOpenAI(req, m)
	assert.NoError(t, err, "should not error on text-only tool_result")
	assert.Len(t, out.Messages, 1, "should have tool message")

	var text string
	err = json.Unmarshal(out.Messages[0].Content, &text)
	assert.NoError(t, err, "text-only content should stay a bare string")
	assert.Equal(t, "Line one\n\nLine two", text, "text blocks should concatenate")
}
