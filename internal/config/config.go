...
// Setting is one overlay key. A element with child elements carries them in
...
// claudeAiImport, so a rejected overlay surfaces here by name instead of as a
// silently missing feature.
//
// The endpoint trio is an override for a self-hosted export service and must be
// set together or left entirely empty. Leaving it empty with enabled=true is
// the normal case: the app imports from Claude.ai using its own endpoints.
